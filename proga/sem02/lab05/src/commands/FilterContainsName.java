...
public class FilterContainsName implements Command {
    ...
    public void execute() {
        try {
            ...
            var filtered = collectionManager.getCollection().stream().filter(city -> city.getName().contains(substring)).collect(Collectors.toList());

            if (filtered.isEmpty()) {
                System.out.println("Элементы с name, содержащим '" + substring + "', не найдены.");
            } else {
