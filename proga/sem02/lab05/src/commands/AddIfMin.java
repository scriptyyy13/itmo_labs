...
public class AddIfMin implements Command {
    ...
    public void execute() {
        try {
            ...
            Optional<City> minCityOpt = collectionManager.getCollection().stream().min(Comparator.naturalOrder());
            // сравниваем по population через Comparable<City>

            if (minCityOpt.isEmpty() || newCity.getPopulation() < minCityOpt.get().getPopulation()) {
                collectionManager.add(newCity);
                System.out.println("Город добавлен (он меньше всех): " + newCity);
            } else {
                System.out.println("Город не добавлен, так как его population не меньше наименьшего в коллекции.");
            }
